 ...
 * powered by Claude — making your templates/agents much more valuable.
 */
...
async function generateContent({ prompt, type, database_id, tone }) {
  ...
  // If database_id provided, fetch schema for context
  if (database_id) {
    try {
  ...
  const systemPrompt = `You are a content creator. Generate ${contentType} content with a ${contentTone} tone.${contextInfo}
...
Provide a clear, data-driven answer with specific numbers and insights. Use markdown tables if helpful.`,
    }],
  });
...
async function smartSearch({ query, summarize }) {
  ...
  // Fetch content of top 3 results and summarize
...
async function draftFromTemplate({ template_type, topic, details }) {
  ...
  const response = await client.messages.create({
    model: config.claudeModel,
    max_tokens: 2048,
    system: 'You are a professional content writer. Generate well-structured, ready-to-use Notion page content in markdown format.',
    messages: [{
      role: 'user',
