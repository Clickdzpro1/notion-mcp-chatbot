...
async function startWorkflowFromIntent(history, msg, sessionId, intent, ctx) {
  ...
  if (result.error) {
    return respond(history, msg, `❌ ${result.error}`, null, []);
  }
  ...
  if (extractedName && result.workflow.steps[0]?.type === 'title') {
    ...
    if (result.workflow.currentStep >= result.workflow.steps.length) {
      ...
      return respond(history, msg, completeResult.message || '✅ Created!', null, []);
    }
    ...
    return respond(history, msg, promptText, {
  ...
  return respond(history, msg, prompt.text, prompt.structured, suggestions);
}
