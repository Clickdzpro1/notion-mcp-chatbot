...
async function startWorkflowFromIntent(history, msg, sessionId, intent, ctx) {
  ...
  // If no database resolved from cache, try matching workflow template keywords + registry
  if (!dbId) {
    const tokens = msg.toLowerCase().split(/\s+/);

    // Try built-in workflow templates first
    ...
    // Try registry natural triggers
    if (!dbId) {
      const regEntry = workflowEngine.findDatabaseByTrigger(msg.toLowerCase());
      if (regEntry) {
        const allDbs = dbCache.getAll();
        for (const db of allDbs) {
          const dbLower = db.title.toLowerCase();
          if (regEntry.keywords.some(k => dbLower.includes(k))) {
            dbId = db.id;
            break;
          }
        }
