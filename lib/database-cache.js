...
async function refresh() {
  try {
    ...
    // Also index from the registry — match registry entries to actual databases
    for (const regEntry of registry.getAll()) {
      // Find the actual database that matches this registry entry
      const matchedDb = databases.find(db => {
        const dbLower = `${db.title} ${db.description || ''}`.toLowerCase();
        return regEntry.keywords.some(k => dbLower.includes(k));
      });

      if (matchedDb) {
        // Index ALL registry keywords → this database ID
        for (const kw of regEntry.keywords) {
          nameIndex[kw] = matchedDb.id;
        }
        // Index natural triggers
        for (const trigger of regEntry.naturalTriggers) {
          const words = trigger.toLowerCase().split(/\s+/);
          for (const w of words) {
            if (w.length >= 3 && !nameIndex[w]) nameIndex[w] = matchedDb.id;
          }
        }
        // Store registry metadata on the database object
        matchedDb.registry = regEntry;
      }
    }
