...
function checkAuth(req, res) {
  // Auth disabled for now — enable by setting MCP_AUTH_TOKEN env var
  if (!config.mcpToken) return true;

  const authHeader = req.headers['authorization'];
  if (!authHeader) {
    // Allow requests without auth header when token is set
    // (some MCP clients don't send auth on every request)
    console.log('  [AUTH] No auth header, but MCP_AUTH_TOKEN is set — allowing for now');
    return true;
  }
