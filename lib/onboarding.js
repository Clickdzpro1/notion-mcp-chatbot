...
// Use the full registry for expected databases
const EXPECTED_DATABASES = registry.getAll().map(entry => ({
  keywords: entry.keywords,
  emoji: entry.emoji,
  name: `${entry.emoji} ${entry.keywords[0]}`,
...
async function stepWorkspaceScan(state) {
  ...
  let statusEmoji = score >= 90 ? '🟢' : score >= 70 ? '🟡' : '🔴';
  ...
  const catSummary = Object.entries(byCat).map(([catKey, dbs]) => {
    const cat = categories[catKey];
    return `${cat?.emoji || '📁'} **${cat?.nameEn || catKey}** — ${dbs.length} databases`;
  }).join('\n');
...
✅ **${detected.length}/${EXPECTED_DATABASES.length}** databases detected (${score}%)

${catSummary}

${missing.length > 0 ? `\n_${missing.length} databases not found — they may use different names._` : '\n🎯 **Full setup detected!**'}
